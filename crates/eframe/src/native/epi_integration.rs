...
pub fn viewport_builder(
    ...
    if native_options.centered {
        ...
        {
            let monitor_size = monitor.current_video_mode().unwrap().size()
...
fn largest_monitor_point_size(egui_zoom_factor: f32, event_loop: &dyn ActiveEventLoop) -> egui::Vec2 {
    ...
    for monitor in available_monitors {
        let size = monitor.current_video_mode().unwrap().size()
            .to_logical::<f32>(egui_zoom_factor as f64 * monitor.scale_factor());
...
impl EpiIntegration {
    ...
    pub fn post_rendering(&mut self, window: &Box<dyn winit::window::Window>) {
        profiling::function_scope!();
